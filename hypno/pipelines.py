...
class HypnoPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('hypno.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `hypno`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM hypno WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO hypno (date, title, link, content)"
